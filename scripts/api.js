...
exports.get = function(path, httpOptions, callbackData, callbacks) {
    ...
    return httpService.get(BrighterBins(options), callbackData, callbacks);
};
...
exports.post = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.post(BrighterBins(options), callbackData, callbacks);
};
...
exports.put = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.put(BrighterBins(options), callbackData, callbacks);
};
...
exports.patch = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.patch(BrighterBins(options), callbackData, callbacks);
};
...
exports.delete = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.delete(BrighterBins(options), callbackData, callbacks);
};
...
exports.head = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.head(BrighterBins(options), callbackData, callbacks);
};
...
exports.options = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.options(BrighterBins(options), callbackData, callbacks);
};
...
exports.utils = {
    ...
    getConfiguration: function (property) {
        if (!property) {
            sys.logs.debug('[brighterbins] Get configuration');
            return JSON.stringify(config.get());
        }
        sys.logs.debug('[brighterbins] Get property: '+property);
...
let BrighterBins = function (options) {
    options = options || {};
 ...
 ****************************************************/
 ...
 const TOKEN_KEY = 'brighterbins:token';
 const TOKEN_EXPIRATION_KEY = 'brighterbins:token-expiration';

 function getAccessToken() {
    sys.logs.debug('[brighterbins] Getting access token');
    let token = sys.storage.get(TOKEN_KEY, { decrypt: true });
    let expiration = sys.storage.get(TOKEN_EXPIRATION_KEY);
    if (! token) sys.logs.info('[brighterbins] No token found in storage');
    if (expiration < new Date().getTime()) sys.logs.info(`[brighterbins] expiration was ${expiration}, now is ${new Date().getTime()}`);
    if (! token || expiration < new Date().getTime()) {
        sys.logs.info('[brighterbins] Access token is expired or not found. Getting new token');
        let res = httpService.post(
            {
                url: config.get('BRIGHTERBINS_API_BASE_URL') + '/auth/login',
                headers: {
        ...
        token = res.token;
        let expiresAt = (new Date().getTime()) + (3 * 24 * 60 * 60 * 1000);
        sys.logs.info(`[brighterbins] Saving new token for account: ${res.name} - ${res.company} - Expires at ${new Date(expiresAt)}`);
        sys.storage.put(TOKEN_KEY, token, { encrypt: true });
        sys.storage.put(TOKEN_EXPIRATION_KEY, expiresAt);
    }
    return token;
 }
...
function setApiUri(options) {
    ...
    const API_URL = config.get('BRIGHTERBINS_API_BASE_URL')
    options.url = API_URL + url;
    sys.logs.debug('[brighterbins] Set url: ' + options.path + '->' + options.url);
    return options;
}
